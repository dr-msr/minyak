/**
 * LogEntry component to display and manage fuel log entries.
 *
 * Allows user to enter odometer reading, select fuel type,
 * amount and price to log an entry. Fetches latest fuel
 * price data from API.
 */
...
import { getPrice } from '@/lib/server';
...
export const LogEntry = () => {
	...
	function initAdd() {
		if (inputOdonto <= 0) {
			toast('Odometer cannot be empty or negative.');
			return;
		}
		if (selectedValue == '') {
		...
		if (inputAmount <= 0) {
			toast('Amount cannot be empty or negative.');
			return;
		}
		...
		if (data[data.length - 1] != undefined) {
			if (inputOdonto <= data[data.length - 1].odometer) {
				toast(
					'Odometer reading cannot be same or lower than previous record.'
				);
				return;
			}
		}
		...
		var trip =
			data.length > 0 ? inputOdonto - data[data.length - 1].odometer : 0;
		var consumption =
			data.length > 0
				? trip /
				  getLastFuel(
						data[data.length - 1].amount,
						data[data.length - 1].ron
				  )
				: 0;
		...
		const result =
			context.data.Log.length > 1
				? 'Fueling record has been created. You has completed ' +
				  trip +
				  'km journey with ' +
		...
		if (harga) {
			...
			toast.success(
				'Price updated successfully : ' +
					new Date(harga.date).toLocaleDateString('en-MY') +
					' (https://data.gov.my).'
			);
		} else {
			toast.error(
				'Failed to fetch the latest price. Current price is dated at ' +
					new Date(priceData.date).toLocaleDateString('en-MY') +
					'.'
			);
		}
	}
	...
	useEffect(() => {
		switch (selectedValue) {
			case '1':
				setAmount(settingData.preset[1]);
				setShowCustom(false);
				break;
			case '2':
				setAmount(settingData.preset[2]);
				setShowCustom(false);
				break;
			case '3':
				setAmount(settingData.preset[3]);
				setShowCustom(false);
				break;
			case '4':
				setAmount(settingData.preset[4]);
				setShowCustom(false);
				break;
			case '5':
				setShowCustom(true);
	...
	useEffect(() => {
		if (selectedPreset == 'RM') {
			...
			if (selectedRon == 'RON95') {
				setInputLitre(
					parseFloat((inputAmount / priceData.ron95).toFixed(2))
				);
			} else if (selectedRon == 'RON97') {
				setInputLitre(
					parseFloat((inputAmount / priceData.ron97).toFixed(2))
				);
			}
			...
			<CardContent className='space-y-2'>
				...
				<div className='flex justify-between'>
					<div>
						...
						>
							<ToggleGroupItem value='RON95'>
								RON95
							</ToggleGroupItem>
							<ToggleGroupItem value='RON97'>
								RON97
							</ToggleGroupItem>
						</ToggleGroup>
					</div>
				...
				<div className='flex flex-col w-full gap-2 mt-2'>
					<div className='flex flex-row'>
						<div className='flex-grow flex-row'>
							...
							>
								{priceData.ron97 != 0
									? 'RM ' + priceData.ron97 + '/L'
									: 'Loading..'}
							</Badge>
						</div>

						<div>
							<Badge onClick={() => initPrice()}>Update</Badge>
						</div>
					</div>
					<Button
						onClick={() => initAdd()}
