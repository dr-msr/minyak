...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/03/09 12:45:50 by mrahim@42KL       #+#    #+#             */
/*   Updated: 2024/03/09 12:45:54 by mrahim@42KL      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

/**
 * DataConversion component displays a card with options to handle
 * incompatible data structures between the loaded data and latest app version.
 * Provides options to convert, restore from backup, reset, or export data.
 */
...
import { FileCode2, Sheet } from 'lucide-react';
...
const DataConversion: React.FC<DataConversionProps> = ({ success }) => {
	...
	function initConversion(): void {
		if (convertedData) {
			...
			context.emptyData(), toast.success('Data has been reset.');
			success(true);
		}
	}
		...
		<Card className='w-[400px]'>
			...
			<CardContent className='grid gap-4'>
				<div className=' flex items-center space-x-4 rounded-md border p-4'>
					...
					<div className='flex-1 space-y-1'>
						...
						<p className='text-sm text-muted-foreground'>
							We detected that there is an existing data in place.
						</p>
					</div>
				</div>
				...
				<div className=' flex items-center space-x-4 rounded-md border p-4'>
					<div className='flex-1 space-y-1'>
						<div className='text-sm text-muted-foreground flex flex-col gap-2 mb-2 text-justify'>
							...
							<div className='flex flex-row justify-between'>
								<p>Current Version : </p>
								<Badge>{defaultData.latest.Version}</Badge>
							</div>
							...
							<div className='mt-2'>
								{!loadData.Version ? (
									<div>
										<div>
											Unable to detect the data structure.
										</div>
										<div className='mt-1.5'>
											Please choose {'['}Restore{']'} to
											load data from existing savefile or{' '}
											{'['}Initiate Fresh Data{']'} to
									...
									<div>
										<div>
											A conversion tool is not available.
										</div>
										<div className='mt-1.5'>
											Please choose {'['}Restore{']'} to
											load data from existing savefile or{' '}
				...
				<Command className='rounded-lg border shadow-md'>
					<CommandList>
						<CommandGroup
							heading={
								loadData.Version ? 'Data Savefile' : 'Restore'
							}
						>
							{loadData.Version && (
								<CommandItem onSelect={() => initBackup()}>
							...
							<Dialog
								open={closeRestore}
								onOpenChange={setCloseRestore}
							>
								<DialogTrigger asChild>
									<CommandItem
										onSelect={() => setCloseRestore(true)}
									>
			...
			<CardFooter>
				<Button className='w-full' onClick={() => initConversion()}>
					...
					{convertedData ? 'Begin Conversion' : 'Initiate Fresh Data'}
				</Button>
			</CardFooter>
		</Card>
	);
