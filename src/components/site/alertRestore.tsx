/**
 * AlertRestore component displays a confirmation dialog for restoring data from a savefile.
 * Allows user to upload a savefile, parses & validates the data, displays a summary,
 * and prompts the user to confirm before initializing the app's data context.
 */
...
export const AlertRestore: React.FC<AlertRestoreProps> = ({ success }) => {
	...
	const [parsedData, setParsedData] = useState<DataType['latest'] | null>(
		null
	);
		...
		restoreFile(input)
			.then((data) => {
				if (data.Version == undefined) {
					toast.error(
						'Unable to find data version. Savefile data is not valid.'
					);
					return;
				}
			...
			.catch((error) => {
				console.log(error);
				toast.error(
					'Unable to read file. Make sure you uploaded the correct savefile.'
				);
			});
	}
			...
			<AlertDialog open={isOpen}>
				<AlertDialogContent>
						...
						<AlertDialogDescription>
							{parsedData ? (
								<div className='w-full p-2 border text-left max-h-[300px] overflow-scroll'>
									<p>
										<code>Loading... {filename} </code>
									</p>
									<p>
										<code>
											Savefile version : {fileversion}{' '}
											{'('}Latest :{' '}
											{defaultData.latest.Version}
											{')'}
										</code>
									</p>
									<p>
									...
									<p>
										<code>
											{'>> '} Total{' '}
											{parsedData.Log.length} rows loaded.
										</code>
									</p>
									<p>
										<code>==========</code>
								...
								<div className='w-full p-2 border text-left'>
									...
									<p>
										<code>
											Your savefile version is outdated.
											Unfortunately no conversion tool is
											available. Please reach out to the
											developer to manually convert your
											data.{' '}
										</code>
									</p>
								</div>
					...
					<AlertDialogFooter>
						...
						<AlertDialogAction onClick={handleInit}>
							Continue
						</AlertDialogAction>
					</AlertDialogFooter>
				</AlertDialogContent>
